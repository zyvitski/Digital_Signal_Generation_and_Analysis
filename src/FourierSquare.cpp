...
DSG::FourierSquare::FourierSquare():FourierGenerator(),_a((4.0/PI)*0.85),_h(_maxHarms(_frequency)+1),phs(0),stor(0){
}
DSG::FourierSquare::FourierSquare(double const& frequency,double const& phase_offset):FourierGenerator(frequency,phase_offset),_a((4.0/PI)*0.85),_h(_maxHarms(_frequency)+1),phs(0),stor(0){
    _sample=0;
    _storage=0;
}
